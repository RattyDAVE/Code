...
void updatemap(int angle, int distance){
   ...
   if ((newX < points) && (newY < points) && (newX > 0) && (newY > 0)){
     ...
     for (countdown = 0; countdown < distance - 1; countdown = countdown 
+ 1) {
...
void waveFill() {
   int minVal;
   int reset_minVal = 13;  //Lowest value to go to. 1-10 = Used for 
Objects, 11 = Robot, 240 = goal
   int i;
   int y;
   for (i = 0; i < 239; i++)   //Scroll through find solution "i" times, 
IF IT LEAVES WHITE WITHOUT SOLVING INCREASE THIS
   {
     for (x = 0; x < points; x++)   //Scroll down the MAP Array
     {
       for (y = 0; y < points; y++)   //Scroll across the MAP Array
       {
         ...
         if (mapArray[x][y] == 0 || mapArray[x][y] == 11) //THEN FIND THE 
HIGHEST VALUE AROUND CURRENT COORDINATE
         {
...
***************************************************************
           if (x < points - 1)//not out of boundary
             ...
             {   //IF TO THE RIGHTS VALUE IS GREATER THAN minVal & IS IN 
RANGE
...
****************************************************************
           if (x > 0)
             if (mapArray[x - 1][y] > minVal && mapArray[x - 1][y] < 241)
             {   //IF TO THE LEFTS VALUE IS GREATER THAN minVal & IS IN 
RANGE
               minVal = mapArray[x - 1][y];
             }
           //Down  
...
*******************************************************************
           ...
           //If Value > reset_minVal && Location is a Robot. SOLVED & 
EXIT
           if (minVal > reset_minVal && mapArray[x][y] == 11)
           {
             return;
           }
           else if (minVal != reset_minVal)  //If Value doesn't equal the 
reset value change current coordinate with the min value found - 1
           {
   ...
   //0 = Clear, 1-10 = Object, 11 = Robot, 240 = Goal, Above 240 Reserved 
for Upgrades
   bitClear(flags, 1); // Clear the Flag on Bit 1. We have done the 
routing.
}
void clearMap() {
   for (x = 0; x < points; x++)   //Scroll down the MAP Array
   {
     for (y = 0; y < points; y++)   //Scroll across the MAP Array
     {
       if (mapArray[x][y] > 11 && mapArray[x][y] < 241)  //If the map 
Coordinate is between 11 & 240 reset to Zero
       {
         mapArray[x][y] = 0;
       }
     }
...
mapArray[currentX][currentY] = 11;   //Mark current position on map with 
a 11 for debuging.
