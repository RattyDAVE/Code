...
//global variables
byte intensity = 0;                      // Default intensity/brightness (0-15)
...
void slide() {
  ...
  byte digits_old[4] = {
    99, 99, 99, 99  }; //old values  we store time in. Set to somthing that will never match the time initially so all digits get drawn wnen the mode starts
  byte digits_new[4]; //new digits time will slide to reveal
  byte digits_x_pos[4] = {
    25, 19, 7, 1  }; //x pos for which to draw each digit at
...
void scroll() {
  ...
  byte chara[] = {
    0, 1, 2, 3, 4, 5  }; //chars from string
  int x[] = {
    0, 6, 12, 18, 24, 30  }; //xpos for each char
  byte y = 0;                   //y pos
